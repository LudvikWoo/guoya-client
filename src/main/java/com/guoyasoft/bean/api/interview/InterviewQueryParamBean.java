package com.guoyasoft.bean.api.interview;

public class InterviewQueryParamBean {
	private String studentId;
	private String custmerName;
	private String classId;
	private String companyName;
	private String hrName;
	private String interviewDateStart;
	private String interviewDateEnd;
	private String interviewTime;
	private String workTimeStart;
	private String workTimeEnd;
	private String progress;
	private String result;
	private String salayMin;
	private String salayMax;
	private String orderField;
	private String orderType;
	public String getStudentId() {
		return studentId;
	}
	public void setStudentId(String studentId) {
		this.studentId = studentId;
	}
	public String getCustmerName() {
		return custmerName;
	}
	public void setCustmerName(String custmerName) {
		this.custmerName = custmerName;
	}
	public String getClassId() {
		return classId;
	}
	public void setClassId(String classId) {
		this.classId = classId;
	}
	public String getCompanyName() {
		return companyName;
	}
	public void setCompanyName(String companyName) {
		this.companyName = companyName;
	}
	public String getHrName() {
		return hrName;
	}
	public void setHrName(String hrName) {
		this.hrName = hrName;
	}
	public String getInterviewDateStart() {
		return interviewDateStart;
	}
	public void setInterviewDateStart(String interviewDateStart) {
		this.interviewDateStart = interviewDateStart;
	}
	public String getInterviewDateEnd() {
		return interviewDateEnd;
	}
	public void setInterviewDateEnd(String interviewDateEnd) {
		this.interviewDateEnd = interviewDateEnd;
	}
	public String getInterviewTime() {
		return interviewTime;
	}
	public void setInterviewTime(String interviewTime) {
		this.interviewTime = interviewTime;
	}
	public String getWorkTimeStart() {
		return workTimeStart;
	}
	public void setWorkTimeStart(String workTimeStart) {
		this.workTimeStart = workTimeStart;
	}
	public String getWorkTimeEnd() {
		return workTimeEnd;
	}
	public void setWorkTimeEnd(String workTimeEnd) {
		this.workTimeEnd = workTimeEnd;
	}
	public String getProgress() {
		return progress;
	}
	public void setProgress(String progress) {
		this.progress = progress;
	}
	public String getResult() {
		return result;
	}
	public void setResult(String result) {
		this.result = result;
	}
	public String getSalayMin() {
		return salayMin;
	}
	public void setSalayMin(String salayMin) {
		this.salayMin = salayMin;
	}
	public String getSalayMax() {
		return salayMax;
	}
	public void setSalayMax(String salayMax) {
		this.salayMax = salayMax;
	}
	public String getOrderField() {
		return orderField;
	}
	public void setOrderField(String orderField) {
		this.orderField = orderField;
	}
	public String getOrderType() {
		return orderType;
	}
	public void setOrderType(String orderType) {
		this.orderType = orderType;
	}
	
}
